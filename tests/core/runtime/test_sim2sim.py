...
from talon_rl.deployment.phase1 import (
    ...
    OBS_DIM,
    build_canonical_obs,
    root_com_velocity_b_from_freejoint,
)
...
def test_build_a1_actor_obs_reorders_mujoco_joint_state_into_isaac_lab_order():
    ...
    q0 = obs_cfg.base_lin_vel_dim + obs_cfg.base_ang_vel_dim + obs_cfg.projected_gravity_dim + obs_cfg.command_dim
    # joint_pos is relative to the canonical default pose -- add it back.
    joint_pos_in_obs = obs[q0 : q0 + 12] + CANONICAL_DEFAULT_Q
    expected = np.arange(12, dtype=np.float32)[MUJOCO_TO_ISAAC_PERM]
    np.testing.assert_allclose(joint_pos_in_obs, expected, atol=1e-6)
    # Concretely: MuJoCo's joint 0 (FR_hip) must land at Isaac Lab's
    # obs index 1 (FR_hip is _ISAAC_LAB_JOINT_ORDER[1]), not index 0.
    assert abs(joint_pos_in_obs[1]) < 1e-6
...
def test_talon_obs_layout_is_the_canonical_phase1_48d_vector():
    """The 2026-09-26 obs change exists so one vector serves the Talon env,
    the stock Phase-1 env and D3 deployment. Fails if ObservationSpaceCfg or
    build_a1_actor_obs drift from deployment/phase1.build_canonical_obs --
    order, relative joint pos, or the COM (not free-joint origin) velocity."""
    obs_cfg = ObservationSpaceCfg()
    assert obs_cfg.total_dim - obs_cfg.preference_dim == OBS_DIM
    assert obs_cfg.command_slice == slice(9, 12)
    ...
    data.qpos[7:19] += rng.normal(scale=0.1, size=12)
    data.qvel[:] = rng.normal(size=model.nv)
    mujoco.mj_forward(model, data)
    prev_action = rng.normal(size=12).astype(np.float32)
    command = np.array([0.4, -0.1, 0.2], dtype=np.float32)
    w = np.full(obs_cfg.preference_dim, 1.0 / obs_cfg.preference_dim, dtype=np.float32)

    ours = build_a1_actor_obs(model, data, prev_action, command, w)[:OBS_DIM]

    quat = data.qpos[3:7]
    com_b = np.array(model.body_ipos[model.body("trunk").id])
    lin_b = root_com_velocity_b_from_freejoint(quat, data.qvel[0:3], data.qvel[3:6], com_b)
    ref = build_canonical_obs(
        lin_b, data.qvel[3:6], quat_rotate_inverse_wxyz(quat, np.array([0.0, 0.0, -1.0])),
        command,
        data.qpos[7:19][MUJOCO_TO_ISAAC_PERM], data.qvel[6:18][MUJOCO_TO_ISAAC_PERM], prev_action,
        joint_names=tuple(n + "_joint" for n in _ISAAC_LAB_JOINT_ORDER),
    )
