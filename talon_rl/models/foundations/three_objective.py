"""Three-objective shared actor/critic primitives for V1-C smoke and training."""
...
NUM_OBJECTIVES = len(OBJECTIVE_ORDER)


class V1CSharedActorCritic(ActorCritic):
    """Single shared actor conditioned directly on the preference simplex."""
...
def initialize_from_rsl_m01(model: V1CSharedActorCritic, checkpoint, device: str = "cpu") -> None:
    """Function-preserving expansion of an rsl_rl M0.1 actor/critic.

    Preference columns in the first actor/critic layers are zero-initialized;
    learned std is restored as log(std), matching the existing M0.2 bridge.
    """
    state = torch.load(checkpoint, map_location=device, weights_only=False)
    source = state.get("model_state_dict", state.get("model", state))
    ...
    # The scalar baseline critic cannot initialize three distinct heads
    # differently without introducing a new hypothesis; copy it to each head.
...
def vector_gae(reward: Tensor, value: Tensor, next_value: Tensor, done: Tensor, gamma: float = 0.99, lam: float = 0.95):
    if reward.ndim != 3 or value.shape != reward.shape or next_value.shape != reward.shape[1:] or done.shape != reward.shape[:2]:
...
def scalarized_late_weighted_ppo(ratio: Tensor, advantages: Tensor, w: Tensor, clip_eps: float = 0.2) -> Tensor:
    if advantages.ndim != 2 or advantages.shape[1] != NUM_OBJECTIVES or w.shape != advantages.shape or ratio.shape != advantages.shape[:1]:
        raise ValueError("ratio/advantages/w shape mismatch")
    clipped = ratio.clamp(1.0 - clip_eps, 1.0 + clip_eps)
    per_objective = torch.minimum(ratio[:, None] * advantages, clipped[:, None] * advantages)
    return -(NUM_OBJECTIVES * (w * per_objective).sum(-1)).mean()


def vector_value_loss(value: Tensor, returns: Tensor) -> Tensor:
    if value.shape != returns.shape or value.shape[-1] != NUM_OBJECTIVES:
        raise ValueError("value/returns shape mismatch")
