...
scripts/analysis/diagnostics/ (git mv where tracked, plain mv for scripts
...
Usage:
    PYTHONPATH="$(pwd):$(pwd)/scripts" python scripts/analysis/run_diagnostics.py <experiment> [experiment args...]
    PYTHONPATH="$(pwd):$(pwd)/scripts" python scripts/analysis/run_diagnostics.py --list

Example (2D.2's calf-target sweep):
    python scripts/analysis/run_diagnostics.py static-standing --num_envs 64 --steps 120 \\
...
# subcommand (kebab-case) -> module name under scripts.analysis.diagnostics.
...
_REGISTRY = {
    ...
    "compare-consecutive-rollouts": "compare_consecutive_rollouts",
    "compare-gradients": "compare_gradients",
    "compare-valuation": "compare_valuation",
    "fall-cycle-analysis": "fall_cycle_analysis",
    "full-update-trace": "full_update_trace",
    ...
    "mean-update-decomposition": "mean_update_decomposition",
    "minibatch-composition-trace": "minibatch_composition_trace",
    "multi-update-trace": "multi_update_trace",
    "objective-perturbation-probe": "objective_perturbation_probe",
    "objective-segment-audit": "objective_segment_audit",
    "open-loop-step-response": "open_loop_step_response",
    "ppo-update-diagnostic": "ppo_update_diagnostic",
    ...
    "stochastic-diversity-probe": "stochastic_diversity_probe",
    "torque-authority-ablation": "torque_authority_ablation",
}


def _print_list() -> None:
    print("Available diagnostics (scripts/analysis/run_diagnostics.py <name> [args...]):")
...
def main() -> None:
    ...
    module = importlib.import_module(f"scripts.analysis.diagnostics.{module_name}")
    module.main()
