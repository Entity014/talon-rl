...
def main():
 ...
 try:
  ...
  for tag,(run,lam) in RUNS.items():
   ...
   for bi,lab in enumerate(ORDER):
    ...
    for snap in SNAPS:
     ...
     _,ret=vector_gae(rt,vt,nv,dt,lam=lam);RET=ret.cpu().numpy()
  ...
  p=ROOT/"runs/post_v2_t5_c8_lambda1-2026-09-23/target_gap_audit.json";p.write_text(json.dumps(out,indent=2)+"\n")
  for s in SNAPS:
   for tag in RUNS:
    a=[];b=[]
