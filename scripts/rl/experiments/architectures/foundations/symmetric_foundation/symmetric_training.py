...
OUT_DEFAULT=ROOT/"runs/foundation_v2_symmetric"
ORDER=("T","A","O","S","C")
...
def main():
    ...
    ap.add_argument("--arch",choices=["rv1","v2a"],required=True)
    ...
    try:
        ...
        m=(T4SharedActorCritic(o.shape[-1],ad) if args.arch=="rv1" else V2AMinimalEmbeddingActorCritic(o.shape[-1],ad)).cuda()
        init_path=RV1_INIT if args.arch=="rv1" else V2A_INIT
        ...
        actor_params=[p for n,p in m.named_parameters() if n.startswith("actor_") or n=="log_std" or (args.arch=="v2a" and n.startswith("preference_embedding"))]
        ...
        report={"schema":"foundation_v2_symmetric_train_v1","seed":args.seed,"updates":args.updates,
                "training_scope":"Foundation V2 expanded current-policy support; identical rule for RV1 and V2-A",
                "actor_ppo_objectives_changed":False,"architecture":args.arch,"anchor_specs":anchor_specs,"rows":rows,"snapshots":snaps}
        ...
        report["summary"]={"status":"FOUNDATION-V2 INTERNAL PASS" if all(criteria.values()) else "FOUNDATION-V2 INTERNAL FAIL",
                           "criteria":criteria,"final":final,"last10_termination_fraction":float(term[-10:].mean()),
                           "max_ratio_error":float(ratio.max())}
        (args.output_dir/"foundation_v2_report.json").write_text(json.dumps(report,indent=2)+"\n")
