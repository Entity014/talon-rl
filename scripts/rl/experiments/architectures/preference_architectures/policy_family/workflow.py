...
def run_v2pf_h0_function_preservation():
    """Run former v2pf_h0_function_preservation.py stage."""
    ...
    OUT=ROOT/'runs/v2pf_h0_function_preservation-2026-09-25';OUT.mkdir(parents=True,exist_ok=True)
    PREFS={'T':np.array([.7,.1,.1,.1],np.float32),'A':np.array([.1,.7,.1,.1],np.float32),
    'O':np.array([.1,.1,.7,.1],np.float32),'S':np.array([.1,.1,.1,.7],np.float32),'C':np.array([.25,.25,.25,.25],np.float32)}
    SEEDS=(840001,840002,840003,840004);TOL=1e-6
    def ot(x):
    ...
    def main():
        ...
        try:
            ...
            from talon_rl.models.conditioning.policy_family import V2PFPolicyFamilyActorCritic,initialize_from_v2b
            ...
            b=V2BSingleSiteFiLMActorCritic(o.shape[-1],ad).cuda();b.load_state_dict(state);b.eval()
            m=V2PFPolicyFamilyActorCritic(o.shape[-1],ad).cuda();initialize_from_v2b(m,state);m.eval()
            maxes={k:0.0 for k in ('action','stochastic_action','logp','value','ratio','env_action','roundtrip_action')};rows=[]
            init=OUT/'v2pf_h0_init.pt';torch.save({'model':m.state_dict(),'base_checkpoint':str(BASE.relative_to(ROOT))},init)
            mr=V2PFPolicyFamilyActorCritic(o.shape[-1],ad).cuda();mr.load_state_dict(torch.load(init,map_location='cuda',weights_only=False)['model']);mr.eval()
            for seed in SEEDS:
                cur,_=env.reset(seed=seed);cur=ot(cur).cuda()
                for lab,w_np in PREFS.items():
                    w=torch.tensor(w_np,device='cuda').repeat(len(cur),1)
                    with torch.no_grad():
                        ab=b.act_inference_with_preference(cur,w);am=m.act_inference_with_preference(cur,w);ar=mr.act_inference_with_preference(cur,w)
                        vb=b.value_with_preference(cur,w);vm=m.value_with_preference(cur,w)
                    vals={'action':float((ab-am).abs().max().cpu()),'value':float((vb-vm).abs().max().cpu()),'roundtrip_action':float((am-ar).abs().max().cpu())}
                    torch.manual_seed(seed+1000+list(PREFS).index(lab));sb,lpb,ub=b.act_with_preference_latent(cur,w)
                    torch.manual_seed(seed+1000+list(PREFS).index(lab));sm,lpm,um=m.act_with_preference_latent(cur,w)
                    vals['stochastic_action']=float((sb-sm).abs().max().detach().cpu());vals['logp']=float((lpb-lpm).abs().max().detach().cpu())
                    with torch.no_grad():
                        lpb2=b.logp_from_pre_tanh_with_preference(cur,w,ub);lpm2=m.logp_from_pre_tanh_with_preference(cur,w,ub)
                        rb=torch.exp(lpb2-lpb.detach());rm=torch.exp(lpm2-lpb.detach())
                    vals['ratio']=max(float((rb-1).abs().max().cpu()),float((rm-1).abs().max().cpu()),float((rb-rm).abs().max().cpu()))
                    cur1,_=env.reset(seed=seed+77);cur1=ot(cur1).cuda();a1=b.act_inference_with_preference(cur1,w).detach();n1,_,_,_,_=env.step(a1);obs1=ot(n1).detach().cpu()
                    cur2,_=env.reset(seed=seed+77);cur2=ot(cur2).cuda();a2=m.act_inference_with_preference(cur2,w).detach();n2,_,_,_,_=env.step(a2);obs2=ot(n2).detach().cpu()
                    vals['env_action']=max(float((a1-a2).abs().max().cpu()),float((obs1-obs2).abs().max()))
                    for k,v in vals.items(): maxes[k]=max(maxes[k],v)
                    rows.append({'seed':seed,'preference':lab,**vals})
            wc=torch.tensor(PREFS['C'],device='cuda').repeat(len(o),1)
            coeff=m.family_coefficients(wc);delta=m.generated_parameter_vector(wc)
            m.zero_grad(set_to_none=True);m._conditional_actor_mean(o,wc).sum().backward()
            g=float(m.family_hyper[-1].weight.grad.norm().detach().cpu()) if m.family_hyper[-1].weight.grad is not None else 0.0
            stats={'coeff_abs_max':float(coeff.abs().max().detach().cpu()),'generated_delta_abs_max':float(delta.abs().max().detach().cpu()),
                   'family_hyper_output_grad_norm':g,'family_parameter_count':sum(p.numel() for n,p in m.named_parameters() if n.startswith('family_')),
                   'base_actor_parameter_count':sum(p.numel() for n,p in b.named_parameters() if n.startswith('actor_') or n.startswith('preference_') or n=='log_std')}
            passed=all(v<=TOL for v in maxes.values()) and stats['coeff_abs_max']==0 and stats['generated_delta_abs_max']==0 and g>0
            rep={'schema':'v2pf_h0_function_preservation_v1','status':'PASS' if passed else 'FAIL','tolerance':TOL,
                 'base_checkpoint':str(BASE.relative_to(ROOT)),'max_errors':maxes,'family_stats':stats,'rows':rows,'decision':{'H1_authorized':passed}}
            rp=OUT/'v2pf_h0_report.json';rp.write_text(json.dumps(rep,indent=2)+'\n')
            (OUT/'PROVENANCE_MANIFEST.json').write_text(json.dumps({'status':'FROZEN_BY_HASH','report_sha256':sha(rp),'script_sha256':sha(Path(__file__).resolve()),
                 'architecture_sha256':sha(ROOT/'talon_rl/models/conditioning/policy_family.py'),'base_checkpoint_sha256':sha(BASE),'init_checkpoint_sha256':sha(init)},indent=2)+'\n')
...
def run_v2pf_h1_policy_family_screen():
    """Run former v2pf_h1_policy_family_screen.py stage."""
    from pathlib import Path
    ...
    V2PF_INIT=ROOT/"runs/v2pf_h0_function_preservation-2026-09-25/v2pf_h0_init.pt"
    OUT_DEFAULT=ROOT/"runs/v2pf_h1_policy_family-2026-09-25"
    ORDER=("T","A","O","S","C")
    PREFS={"T":np.array([.7,.1,.1,.1],np.float32),"A":np.array([.1,.7,.1,.1],np.float32),
    ...
    def sensitivity(m,probe):
        total={};masked={};coeffs={};params={};pfres={}
        with torch.no_grad():
            for lab in ORDER:
                w=torch.tensor(PREFS[lab],device="cuda").repeat(len(probe),1)
                x=m._actor_features_v2a(probe,w)
                base_mean=m.actor_mean(x)
                ...
                pfres[lab]=total[lab]-masked[lab]
    
        pair_total=pairwise_dist(total);pair_masked=pairwise_dist(masked)
        authority=float(np.mean([torch.linalg.vector_norm(pfres[k],dim=1).mean().cpu() for k in ORDER]))
        heavy=("T","A","O","S")
        cmean={k:coeffs[k].mean(0).cpu().numpy() for k in ORDER}
        pmean={k:params[k].mean(0).cpu().numpy() for k in ORDER}
        coeff_pair=[];param_pair=[]
        for i,a in enumerate(heavy):
            for b in heavy[i+1:]:
                coeff_pair.append(float(np.linalg.norm(cmean[a]-cmean[b])))
                param_pair.append(float(np.linalg.norm(pmean[a]-pmean[b])))
    
        coeff_svd=_svd_summary(np.stack([cmean[k] for k in ORDER]))
        P=np.stack([pmean[k] for k in ORDER]);Pbar=P.mean(0);Pc=P-Pbar
        ps=np.linalg.svd(Pc,compute_uv=False);p_s2=float(ps[1]/(ps[0]+1e-12)) if len(ps)>1 else 0.0
        p_eff=int(np.sum(ps >= (ps[0]*.05 if ps[0]>0 else np.inf)))
        p_single=float((ps[0]**2)/(np.sum(ps**2)+1e-12)) if len(ps) else 1.0
        ecommon=float(np.linalg.norm(Pbar));espec=float(np.sqrt(np.mean(np.sum(Pc*Pc,axis=1))));psfrac=espec/(ecommon+espec+1e-12)
    
        # Functional family residual geometry, centered per state across all five preferences.
        F=np.stack([pfres[k].cpu().numpy() for k in ORDER],axis=0) # [P,B,A]
        Fbar=F.mean(0,keepdims=True);Fc=F-Fbar
        fcommon=float(np.sqrt(np.mean(np.sum(Fbar[0]**2,axis=1))))
        fspec=float(np.sqrt(np.mean(np.sum(Fc**2,axis=2))))
        fsfrac=fspec/(fcommon+fspec+1e-12)
        # SVD over preference rows after flattening states/actions.
        Fflat=Fc.reshape(len(ORDER),-1);fs=np.linalg.svd(Fflat,compute_uv=False)
        f_eff=int(np.sum(fs >= (fs[0]*.05 if fs[0]>0 else np.inf)))
        f_s2=float(fs[1]/(fs[0]+1e-12)) if len(fs)>1 else 0.0
        fpairs=[]
        for i,a in enumerate(ORDER):
            for b in ORDER[i+1:]:
                fpairs.append(float(np.sqrt(np.mean(np.sum((Fc[i]-Fc[list(ORDER).index(b)])**2,axis=1)))))
    
        # Full and family-masked preference Jacobians at center.
        ...
        xm=m._actor_features_v2a(probe,wm);am=torch.tanh(m.actor_mean(xm))*m.ACTION_CLIP;mrows=[]
        ...
        Jp=torch.autograd.functional.jacobian(pf,wc,create_graph=False).detach()
        sc=torch.linalg.svdvals(Jc).cpu().numpy();sp=torch.linalg.svdvals(Jp).cpu().numpy()
        jc={"fro_norm":float(torch.linalg.matrix_norm(Jc,ord="fro").cpu()),"singular_values":sc.tolist(),"s2_over_s1":float(sc[1]/(sc[0]+1e-12)) if len(sc)>1 else 0.0,"effective_rank_5pct":int(np.sum(sc >= (sc[0]*.05 if sc[0]>0 else np.inf)))}
        jp={"fro_norm":float(torch.linalg.matrix_norm(Jp,ord="fro").cpu()),"singular_values":sp.tolist(),"s2_over_s1":float(sp[1]/(sp[0]+1e-12)) if len(sp)>1 else 0.0,"effective_rank_5pct":int(np.sum(sp >= (sp[0]*.05 if sp[0]>0 else np.inf)))}
    
        return {
          "pairwise_action_distance":pair_total,
          "family_masked_pairwise_action_distance":pair_masked,
          "jacobian_fro_mean":float(torch.linalg.matrix_norm(jac,ord="fro",dim=(1,2)).mean().detach().cpu()),
          "family_masked_jacobian_fro_mean":float(torch.linalg.matrix_norm(mjac,ord="fro",dim=(1,2)).mean().detach().cpu()),
          "family_action_authority_mean":authority,
          ...
          "coefficient_svd_all":coeff_svd,
          "generated_parameter_pairwise_distance_heavy":{"mean":float(np.mean(param_pair)),"min":float(np.min(param_pair)),"max":float(np.max(param_pair))},
          "centered_parameter_geometry":{"s2_over_s1":p_s2,"effective_rank_5pct":p_eff,"largest_energy_fraction":p_single,"common_energy":ecommon,"specific_energy":espec,"specific_fraction":psfrac,"singular_values":ps.tolist()},
          "centered_functional_geometry":{"s2_over_s1":f_s2,"effective_rank_5pct":f_eff,"common_rms":fcommon,"specific_rms":fspec,"specific_fraction":fsfrac,"specific_over_common":fspec/(fcommon+1e-12),"pairwise_centered_residual_rms_mean":float(np.mean(fpairs)),"singular_values":fs.tolist()},
          "coefficient_jacobian":jc,"parameter_manifold_jacobian":jp,
        }
    ...
    def main():
        ...
        try:
            ...
            from talon_rl.models.conditioning.policy_family import V2PFPolicyFamilyActorCritic
            ...
            m=V2PFPolicyFamilyActorCritic(o.shape[-1],ad).cuda()
            init_path=V2PF_INIT
            ...
            actor_params=[p for n,p in m.named_parameters() if n.startswith("actor_") or n=="log_std" or n.startswith("preference_embedding") or n.startswith("preference_film") or n.startswith("family_")]
            ...
            pt=sens["pairwise_action_distance"]["mean"];pm=sens["family_masked_pairwise_action_distance"]["mean"]
            jt=sens["jacobian_fro_mean"];jm=sens["family_masked_jacobian_fro_mean"]
            pair_reduce=(pt-pm)/(abs(pt)+1e-12);jac_reduce=(jt-jm)/(abs(jt)+1e-12)
            pg=sens["centered_parameter_geometry"];fg=sens["centered_functional_geometry"]
            criteria={
              "coefficient_separation":sens["coefficient_pairwise_distance_heavy"]["mean"]>=0.02,
              "generated_parameter_separation":sens["generated_parameter_pairwise_distance_heavy"]["mean"]>=0.01,
              "centered_parameter_rank":pg["s2_over_s1"]>=0.10 and pg["effective_rank_5pct"]>=2,
              "centered_parameter_specific_fraction":pg["specific_fraction"]>=0.15,
              "centered_functional_rank_and_fraction":fg["effective_rank_5pct"]>=2 and fg["specific_fraction"]>=0.15,
              "family_action_authority":sens["family_action_authority_mean"]>=1e-3,
              "mask_reduces_preference_authority":max(pair_reduce,jac_reduce)>=0.05,
              "preference_specific_functional_residual":fg["specific_over_common"]>=0.25,
              "coefficient_jacobian_nonzero":sens["coefficient_jacobian"]["fro_norm"]>1e-3,
              "parameter_jacobian_nonzero":sens["parameter_manifold_jacobian"]["fro_norm"]>1e-3,
              "no_single_centered_direction_gt90pct":pg["effective_rank_5pct"]>=2 and pg["largest_energy_fraction"]<=0.90,
              ...
              "full_rank_basis_gradient_observed":float(np.max(bg))>1e-7}
            ...
            if passed:status="V2-PF H1 PASS"
            elif any(not criteria[k] for k in foundation_keys):status="V2-PF H1 FAIL — FOUNDATION"
            else:status="V2-PF H1 FAIL — COMMON FAMILY COLLAPSE"
            report={"schema":"v2pf_h1_policy_family_authority_v1","seed":args.seed,"updates":args.updates,
                    "training_scope":"V2-PF H1 one-model policy-family authority only; semantics blocked",
                    "actor_ppo_objectives_changed":False,"architecture":"v2pf",
                    "reference_checkpoint":str(V2PF_INIT.relative_to(ROOT)),"anchor_specs":anchor_specs,"rows":rows,"snapshots":snaps}
            report["summary"]={"status":status,"criteria":criteria,"initial":initial,"final":final,
              "pairwise_authority_reduction_fraction":pair_reduce,"jacobian_authority_reduction_fraction":jac_reduce,
              "max_family_output_grad_norm":float(np.max(hg)),"median_family_output_grad_norm":float(np.median(hg)),"max_family_basis_grad_norm":float(np.max(bg)),
              "last10_termination_fraction":float(term[-10:].mean()),"max_ratio_error":float(ratio.max()),
              "h2_authorized":bool(passed),"semantic_judgement_performed":False}
            (args.output_dir/"v2pf_h1_report.json").write_text(json.dumps(report,indent=2)+"\n")
...
STAGES = {
    "v2pf_h0_function_preservation": run_v2pf_h0_function_preservation,
    "v2pf_h1_policy_family_screen": run_v2pf_h1_policy_family_screen,
}
