...
MuJoCo snapshot. Two audits take this: one of the scene as configured, one
with base-mass randomisation switched off so the numbers are the nominal
model rather than one sampled instance of it.
"""
import json
import sys
...
class IsaacPlantSnapshot(IsaacAudit):
    ...
    num_envs = 1
    reset_seed = 1
    set_cfg_seed = False      # the originals never set it
