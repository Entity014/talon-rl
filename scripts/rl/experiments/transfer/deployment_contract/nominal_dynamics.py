...
def run_phase1_d3b_extract_commands():
    """Run former phase1_d3b_extract_commands.py stage."""
    ...
    try:
        ...
        for suite in range(4):
            ...
            rows.append({"suite":suite,"seed":seed,"commands":cmd})
        out=ROOT/"runs/phase1_d3b_workload";out.mkdir(parents=True,exist_ok=True)
        (out/"canonical_h2a_commands.json").write_text(json.dumps({"schema":"phase1_d3b_commands_v1","rows":rows},indent=2)+"\n")
        print(json.dumps(rows,indent=2),flush=True)
    finally:
        if env is not None:env.close()
        app.close()

def run_phase1_d3b_nominal_dynamics():
    """Run former phase1_d3b_nominal_dynamics.py stage."""
    from pathlib import Path
    ...
    OUT=ROOT/"runs/phase1_d3b_nominal_dynamics";OUT.mkdir(parents=True,exist_ok=True)
    ...
    def main():
        ...
        rep={"schema":"phase1_d3b_nominal_dynamics_v1","rows":rows,
...
STAGES = {
    "phase1_d3b_extract_commands": run_phase1_d3b_extract_commands,
    "phase1_d3b_nominal_dynamics": run_phase1_d3b_nominal_dynamics,
}
