...
def build_bank(man):
    eng=torch.quasirandom.SobolEngine(3,scramble=True,seed=man["sampler"]["seed"])
    eng.fast_forward(128);u=eng.draw(16).numpy()
    bank=[{"id":"nominal","group":"nominal","mass_delta_kg":0.0,"passive_blend":0.0,"contact_blend":0.0}]
    for i,x in enumerate(u):
        bank.append({"id":f"I{i+1:02d}","group":"in_ensemble",
          "mass_delta_kg":float(-1.5+4.5*x[0]),"passive_blend":float(x[1]),"contact_blend":float(x[2])})
    for x in man["held_out_plants"]:
        y=dict(x);y["group"]="heldout";bank.append(y)
    return bank
...
def apply_matched_reset(env,robot,bank,seed):
    ...
    # Preserve the nominal anchor bit-for-bit: never invoke property setters on lanes 0..7.
    # Phase-4 showed that even ~1e-6 no-op transition perturbations can amplify in closed loop.
    ids=torch.arange(LANES,N,dtype=torch.int32,device="cpu")
...
def main():
    ...
    if nplant!=29:raise RuntimeError(nplant)
    ...
    try:
        ...
        aggregates={}
        for grp in ("nominal","in_ensemble","heldout"):
